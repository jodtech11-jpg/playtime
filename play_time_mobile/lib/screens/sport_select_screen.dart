...
class _SportSelectScreenState extends State<SportSelectScreen> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      if (sportProvider.sports.isEmpty) {
        sportProvider.loadSports(forceReload: true);
      }
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredSports = _searchQuery.isEmpty
        ? sports
        : sports.where((sport) {
            return sport.name.toLowerCase().contains(
                  _searchQuery.toLowerCase(),
                ) ||
                (sport.description?.toLowerCase().contains(
                      _searchQuery.toLowerCase(),
                    ) ??
                    false);
          }).toList();
                          ...
                          child: Container(
                            ...
                            child: Stack(
                              children: [
                                ...
                                ClipRRect(
                                  borderRadius: BorderRadius.circular(24),
                                  child: _buildSportBackground(sport),
                                ),
                                Container(
                                  decoration: BoxDecoration(
                                    borderRadius: BorderRadius.circular(24),
  ...
  Widget _buildSportBackground(Sport sport) {
    final imageUrl = SportUtils.resolveImageUrl(sport);
    final bgColor = SportUtils.parseSportColor(sport.color);
        ...
        children: [
          Image.network(
            ...
            errorBuilder: (context, error, stackTrace) {
              return _sportColorFallback(bgColor, sport.name);
            },
          ),
        ],
      );
  ...
  Widget _sportColorFallback(Color bgColor, String sportName) {
    return Container(
      color: bgColor,
      alignment: Alignment.center,
      child: Icon(
        SportUtils.iconForSport(sportName),
        size: 56,
  ...
  void _showQuickBookModal(VenueProvider venueProvider, List<Venue> venues) {
    ...
    final availableVenues = _selectedSportForQuickBook != null
        ? venues
              .where(
                (venue) => venue.sports.any(
                  (s) =>
                      s.toLowerCase() ==
                      _selectedSportForQuickBook!.toLowerCase(),
                ),
              )
              .toList()
        : venues;
...
class _QuickBookModal extends StatefulWidget {
  ...
  const _QuickBookModal({required this.venues, this.selectedSport});
...
class _QuickBookModalState extends State<_QuickBookModal> {
  ...
  Future<void> _loadCourts() async {
    ...
    try {
      ...
      if (courts.isEmpty &&
          _selectedVenue!.courts != null &&
          _selectedVenue!.courts!.isNotEmpty) {
        courts = _selectedVenue!.courts!
            .map((c) {
              final data = Map<String, dynamic>.from(c);
              final courtId = (data['id'] as String?)?.trim();
              final resolvedId = (courtId != null && courtId.isNotEmpty)
                  ? courtId
                  : '${_selectedVenue!.id}_${(data['name'] as String? ?? 'court').toString().replaceAll(' ', '_').toLowerCase()}';
              data['venueId'] ??= _selectedVenue!.id;
      ...
      if (courts.isNotEmpty) {
        ...
        setState(() {
          ...
          if (activeCourts.isNotEmpty) {
            _selectedCourtId = activeCourts.first.id;
            _loadTimeSlots();
          }
  ...
  Future<void> _loadTimeSlots() async {
    ...
    try {
      ...
      Court? courtOverride;
      try {
        courtOverride = _courts.firstWhere((c) => c.id == _selectedCourtId);
      } catch (_) {
        courtOverride = _courts.isNotEmpty ? _courts.first : null;
      }
  ...
  Future<void> _handleQuickBook() async {
    ...
    try {
      ...
      final (bookingId, isFirstTimeBooking) = await bookingProvider
          .createBooking(
            venueId: _selectedVenue!.id,
