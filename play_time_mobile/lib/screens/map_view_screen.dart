    ...
    for (var venue in processedVenues) {
      ...
      newMarkers.add(
        Marker(
          ...
          position: LatLng(venue.locationLat!, venue.locationLng!),
    ...
    if (!hasChanged) {
      ...
      if (!setEquals(currentIds, newIds)) {
        ...
        for (final newMarker in newMarkers) {
          Marker? oldMarker;
          try {
            oldMarker = _markers.firstWhere(
              (m) => m.markerId == newMarker.markerId,
            );
          } catch (_) {}
          if (oldMarker == null ||
              oldMarker.position != newMarker.position ||
              oldMarker.icon.toJson().toString() !=
                  newMarker.icon.toJson().toString()) {
            hasChanged = true;
            break;
          }
        }
              ...
              builder: (context, snapshot) {
                ...
                return Stack(
                  children: [
                    ...
                    if (!_isSelectingLocation && _markers.isEmpty && _mapReady)
                      Positioned.fill(
                        child: IgnorePointer(
                          child: Container(
                            ...
                            child: Container(
                              ...
                              child: const Column(
                                ...
                                children: [
                                  ...
                                  SizedBox(height: 6),
                                  Text(
                                    'Ask the venue to set a location in the admin panel.',
                                    textAlign: TextAlign.center,
                                    style: TextStyle(
                                      color: Colors.white54,
                                      fontSize: 12,
                ...
                child: Row(
                  children: [
                    ...
                    IconButton(
                      ...
                      onPressed: () {
                        setState(() => _useDarkMapStyle = !_useDarkMapStyle);
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: Text(
                              _useDarkMapStyle
                                  ? 'Dark map style enabled'
                                  : 'Default map style enabled',
                            ),
                        ...
                        if (locationProvider.currentPosition != null) {
                          if (mounted && _mapController != null) {
                            try {
                              ...
                              debugPrint(
                                'Error updating map to current position: $e',
                              );
                            }
                          }
                        } else {
                          ScaffoldMessenger.of(context).showSnackBar(
                            const SnackBar(
                              content: Text(
                                'Could not get GPS location. Please check permissions.',
                              ),
                    ...
                    ? Column(
                        ...
                        children: [
                          ...
                          ElevatedButton.icon(
                            ...
                            style: ElevatedButton.styleFrom(
                              backgroundColor: _isMoving
                                  ? Colors.grey
                                  : AppColors.primary,
                              foregroundColor: _isMoving
                                  ? Colors.white
                                  : AppColors.backgroundDark,
                              padding: const EdgeInsets.symmetric(
                                horizontal: 32,
                ...
                child: Center(
                  child: Column(
                    ...
                    children: [
                      ...
                      const SizedBox(
                        height: 48,
                      ), // Compensate for icon height to keep tip at center
                    ],
                  ),
                ),
              ),
