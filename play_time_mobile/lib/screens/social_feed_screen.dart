...
class _SocialFeedScreenState extends State<SocialFeedScreen> {
  ...
  /// True after the comments stream emitted at least once (loading vs empty).
  final Map<String, bool> _commentsStreamReady = {};
  ...
  void _loadLikeStatusForNewPosts(List<MatchFeedItem> items) {
    ...
    Future.wait(
          itemsToLoad.map(
            (item) => SocialService.hasUserLiked(item.id, user.uid),
          ),
        )
        .then((results) {
          if (mounted) {
        ...
        .catchError((error) {
          debugPrint('Error loading like statuses: $error');
        });
  }
  ...
  Future<void> _handleComment(String postId) async {
    ...
    setState(() {
      ...
      if (_showComments[postId] == true &&
          !_commentControllers.containsKey(postId)) {
    ...
    if (_showComments[postId] == true) {
      ...
      await _commentSubscriptions[postId]?.cancel();
      setState(() => _commentsStreamReady[postId] = false);

      _commentSubscriptions[postId] =
          SocialService.getPostCommentsStream(postId).listen(
            (comments) {
              if (mounted) {
                setState(() {
                  _postCommentsList[postId] = comments;
                  _postComments[postId] = comments.length;
                  _commentsStreamReady[postId] = true;
                });
              }
            },
            onError: (error) {
              ...
              if (mounted) {
                setState(() => _commentsStreamReady[postId] = true);
              }
            },
          );
    } else {
      ...
      _commentsStreamReady.remove(postId);
    }
  }
  ...
  Future<void> _showCreatePostModal() async {
    ...
    await showModalBottomSheet(
      ...
      builder: (context) => Material(
        ...
        child: CreatePostModal(user: user),
      ),
    );
  }
                              ...
                              case 'hide':
                                if (_hiddenPosts.isNotEmpty) {
                                  ...
                                  ScaffoldMessenger.of(context).showSnackBar(
                                    const SnackBar(
                                      content: Text(
                                        'Use the menu on individual posts to hide them',
                                      ),
                                      backgroundColor: AppColors.primary,
                                    ),
                                  );
              ...
              Expanded(
                child: Consumer<FeedProvider>(
                  builder: (context, feedProvider, child) {
                    ...
                    final visibleItems = feedProvider.feedItems
                        .where((item) => !_hiddenPosts.contains(item.id))
                        .toList();

                    // Filter and sort items
                    final filteredItems = _filterAndSortItems(visibleItems);
                    ...
                    return ListView.builder(
                      padding: const EdgeInsets.all(16),
                      itemCount:
                          filteredItems.length + (feedProvider.hasMore ? 1 : 0),
                      itemBuilder: (context, index) {
      ...
      child: Column(
        ...
        children: [
          ...
          if (item.description.isEmpty &&
              item.teamA.name.isEmpty &&
              item.teamB.name.isEmpty)
            Container(
              padding: const EdgeInsets.all(16),
              decoration: BoxDecoration(
                color: AppColors.surfaceDark,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (!(_commentsStreamReady[postId] ?? false))
            const Padding(
          ...
          children: [
            ...
            ElevatedButton(
              onPressed: () {
                Navigator.pop(context);
                Share.share(
                  'Check out PlayTime - The ultimate sports community app!',
                );
              },
              style: ElevatedButton.styleFrom(
                backgroundColor: AppColors.primary,
            ...
            ElevatedButton(
              onPressed: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text(
                      'Use the menu on individual posts to report them',
                    ),
                    backgroundColor: AppColors.primary,
                  ),
                );
          ...
          Container(
            ...
            child: Row(
              ...
              children: [
                Row(
                  children: [
                    Icon(Icons.event, color: AppColors.primary, size: 16),
                    const SizedBox(width: 8),
                    Text(
