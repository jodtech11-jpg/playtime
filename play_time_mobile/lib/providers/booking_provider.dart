...
class BookingProvider with ChangeNotifier {
  ...
  BookingProvider() {
    // Use auth state stream as the single source of truth to avoid double _loadBookings call
  ...
  void _loadBookings(String userId) {
    ...
    _bookingsSubscription?.cancel();
    _bookingsSubscription = FirestoreService.getUserBookingsStream(userId)
        .listen(
          (bookings) {
            _bookings = bookings;
            _isLoading = false;
  ...
  Future<(String, bool)> createBooking({
    ...
    try {
      ...
      // Includes Pending to avoid double-charging if a Pending booking already exists.
      ...
      final bookingId = await FirestoreService.createBookingWithTransaction(
        bookingData,
      );
  ...
  /// One-time fetch for pull-to-refresh; updates list and notifies.
  Future<void> pullToRefresh() async {
    final user = FirebaseAuth.instance.currentUser;
    if (user == null) return;
    try {
      _bookings = await FirestoreService.getUserBookings(user.uid);
      _error = null;
    } catch (e) {
  ...
  Booking? getLatestUpcomingBooking() {
    try {
      return _bookings.firstWhere((b) => b.status.isUpcoming);
    } catch (e) {
      return null;
    }
  }
